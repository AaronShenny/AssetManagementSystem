...
def detect_child_table_changes(doc, old_doc, table_fieldname):
    ...
    old_summary = _summarise_child_table(old_rows)
    new_summary = _summarise_child_table(new_rows)
...
def _summarise_child_table(rows):
    """Return a compact, deterministic text summary of child-table rows."""
    if not rows:
        return ""

    _noise = {
        "name", "owner", "creation", "modified", "modified_by",
        "docstatus", "idx", "parent", "parenttype", "parentfield", "doctype",
    }

    parts = []
    for row in rows:
        d = row.as_dict() if hasattr(row, "as_dict") else dict(row)
        clean = {k: v for k, v in d.items() if k not in _noise}
        parts.append(str(sorted(clean.items())))
